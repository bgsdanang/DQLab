...
ORDER BY SUM(tr_penjualan_detail.qty) DESC
LIMIT 2;
...
ORDER BY SUM(qty*harga_satuan) DESC 
LIMIT 1;
...
ms_pelanggan.alamat
FROM ms_pelanggan
LEFT JOIN tr_penjualan
ON ms_pelanggan.kode_pelanggan = tr_penjualan.kode_pelanggan
GROUP BY ms_pelanggan.kode_pelanggan, ms_pelanggan.nama_pelanggan, ms_pelanggan.alamat
HAVING COUNT(kode_transaksi) = 0;
...
COUNT(qty) AS jumlah_detail
FROM ms_pelanggan
INNER JOIN tr_penjualan
ON ms_pelanggan.kode_pelanggan = tr_penjualan.kode_pelanggan
INNER JOIN tr_penjualan_detail
ON tr_penjualan.kode_transaksi = tr_penjualan_detail.kode_transaksi
GROUP BY tr_penjualan.kode_transaksi, ms_pelanggan.kode_pelanggan, ms_pelanggan.nama_pelanggan, tr_penjualan.tanggal_transaksi
HAVING COUNT(qty) > 1;
